...
SELECT
    COUNT(t.appointid) AS Total_Appointments
FROM
    appointments t
JOIN
    animals a ON t.animalid = a.animalid
JOIN
    owners o ON a.ownerid = o.ownerid
WHERE
    o.ofirstname = 'Maria';
...
SELECT
    a.name,
    COUNT(t.appointid) AS Appointment_Count
FROM
    animals a
JOIN
    appointments t ON a.animalid = t.animalid
GROUP BY
    a.animalid, a.name
ORDER BY
